...
public class ChimeTool extends ItemTool {
	...
	public float getStrVsBlock(ItemStack stack, IBlockState blockState)
	{
		...
		if ((swordEnabled && SWORD_EFFECTIVE_ON.contains(blockState.getBlock())) ||
				(pickEnabled && PICK_EFFECTIVE_ON.contains(blockState.getBlock())) ||
				((axeEnabled || EnchantmentHelper.getEnchantmentLevel(net.minecraft.init.Enchantments.FIRE_ASPECT, new ItemStack(this)) != 0) && AXE_EFFECTIVE_ON.contains(blockState.getBlock())) ||
				(spadeEnabled && SPADE_EFFECTIVE_ON.contains(blockState.getBlock())))
